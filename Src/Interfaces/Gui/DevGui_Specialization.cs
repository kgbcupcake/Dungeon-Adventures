		...
		private void RenderDungeonBuilder()
		{
			...
			if (ImGui.Button("Inject Dungeon into Master", new Vector2(-1, 30)))
			{
				try
				{
					...
					string path = Path.Combine(GameState.MasterPath, "dungeons", $"{_dName.Replace(" ", "_")}.json");
					File.WriteAllText(path, System.Text.Json.JsonSerializer.Serialize(dungeon));
				...
				catch (Exception ex) { DevLog.Write($"Dungeon Injection Failed: {ex.Message}", "ERROR"); }
			}
		}
