	...
	public partial class DevGuiRenderer : Overlay
	{
		...
		private void RenderFullStatEditor()
		{
			...
			if (GameState.CurrentPlayer == null)
			{
				...
				if (ImGui.Button("GENERATE & EJECT NEW PROFILE", new Vector2(-1, 50)))
				{
					...
					SaveGame.ExportToMaster(newHero, "players", _pNameBuffer.Replace(" ", "_"));
				...
				else
				{
					...
					foreach (var file in files)
					{
						...
						if (ImGui.BeginPopupModal("VaporizeConfirm", ref _isDeleteOpen, ImGuiWindowFlags.AlwaysAutoResize))
						{
							...
							if (ImGui.Button("PURGE", new Vector2(100, 0))) { File.Delete(file); ImGui.CloseCurrentPopup(); }
							ImGui.SameLine();
		...
		private void LoadPlayerFromFile(string path)
		{
			try
			{
				...
				var loadedPlayer = JsonSerializer.Deserialize<PlayerData.loadPlayer>(json, new JsonSerializerOptions
				{
					PropertyNameCaseInsensitive = true,
					Converters = { new JsonStringEnumConverter() } // Crucial for your Enums!
				});
			...
			{
				_injectionMsg = $"LOAD ERROR: {ex.Message}";
				_injectionSuccess = false;
			}
			_showInjectionPopup = true;
		}
