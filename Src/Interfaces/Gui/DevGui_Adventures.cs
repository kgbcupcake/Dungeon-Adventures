	...
	public partial class DevGuiRenderer
	{
		public void RenderAdventureCreator()
		{
			...
			if (ImGui.Button("Inject Adventure into Master", new Vector2(-1, 30)))
			{
				try
				{
					...
					string path = Path.Combine(GameState.MasterPath, "adventures", $"{_aMapName.Replace(" ", "_")}.json");
					File.WriteAllText(path, System.Text.Json.JsonSerializer.Serialize(adventure, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
		...
		private void ExecuteAdventureFabrication()
		{
			...
			try
			{
				...
				SaveGame.ExportToMaster(exportData, "adventures", _aMapName.Replace(" ", "_").ToLower());
				GameState.Sync();
				_injectionSuccess = true;
